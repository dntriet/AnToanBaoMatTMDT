<?php
 require_once $_SERVER['DOCUMENT_ROOT']."/Demo_Web_JWT/Project/php/Controller/vendor/autoload.php";
...
function verifyJWT($jwt) {
    ...
    try {
        ...
        $response = [
            ...
            'role_id' => $decodedArray['role'], // Make sure 'role' exists
            'user_id' => $decodedArray['sub'],  // Make sure 'sub' exists
        ];
...
function requestNewAccessToken($user_id, $refreshToken) {
    global $secretKey;
    try {
        $decoded = JWT::decode($refreshToken, new Key($secretKey, "HS256"));
        $jsonRefreshToken = json_encode((array)$decoded);
        if ((array) $jsonRefreshToken['user'] != $user_id) {
            http_response_code(401);
            $response = json_encode([
                'status' => 'error',
        ...
        $role_id = $jsonRefreshToken['role_id'];
