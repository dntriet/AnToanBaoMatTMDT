<?php
    require_once $_SERVER['DOCUMENT_ROOT'].'utils/jwt_utils';
            ...
            $row = $result->fetch_assoc();
                ...
                if (password_verify($password, $passwordFromDatabase)) {
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        $role_id = $row['role_id'];
                        $access_token = generateAccessToken($user_id, $role_id);
                        $refresh_token = generateRefreshToken($user_id, $role_id);
                        $stmt->close();
                        if ($role_id == 1) 
                            // Người dùng có quyền Admin
                            echo json_encode(['status' => 'success', 'role' => 'admin', 'message' =>'Đăng nhập thành công', 'accessToken' => $access_token, 'refreshToken' => $refresh_token]);
                        elseif ($role_id == 2) 
