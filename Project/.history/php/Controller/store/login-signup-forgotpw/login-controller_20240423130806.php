            ...
            $row = $result->fetch_assoc();
                ...
                if (password_verify($password, $passwordFromDatabase)) {
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        $role_id = $row['role_id'];
                        $session_token = generateJWT($user_id, $role_id);
                        $refresh_token = generateRefreshToken($user_id, $role_id);
                        $stmt->close();
                        if ($role_id == 1) 
                            // Người dùng có quyền Admin
                            echo json_encode(['status' => 'success', 'role' => 'admin', 'message' =>'Đăng nhập thành công', 'session_token' => $session_token, 'refresh_token' => $refresh_token]);
                        elseif ($role_id == 2) 
